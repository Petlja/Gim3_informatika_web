...
.. code-block:: javascript
    ...
    function sacuvaj() {
        var podatak = document.querySelector(`#MojModalniDijalog input`).value;
        alert(podatak);
         $('#MojModalniDijalog').modal('hide')
    }
...
Полазећи од кода претходног примера, направите веб страну на у којој се налази дугме за отварање дијалога и дијалог за пољем за унос текста. Када се дијалог отвори и кликне на дугме које би требало да сачува вредност, функционалност у страни треба да прочита унету вредност, да је прикаже и затвори дијалог.

Задатак
'''''''

Направите малу веб страну о Николи Тесли где је текст преузет са Википедија сајта уз 4 слике Николе Тесле. У страни се приказује само једна слика (по избору), а остале су снимљене на истој локацији као и страна.

Када се кликне на слику потребно је отворити дијалог у коме ће бити листа за одабир вредности. Корисник треба да одабере једну од четири слике Николе Тесле које су преузете са Википедија сајта. Када се одабере слика, кôд мора да на основу одабране слике утврди релативну путању до слике и упише је у атрибут ``src`` елемента ``<img>`` у почетној страни. На тај начин корисник ће видети слику коју је изабрао.

Као полазну тачку искористите претходни пример.
